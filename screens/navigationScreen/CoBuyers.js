...
import { StyleSheet, View, Text, Button, TextInput, Image, TouchableOpacity, Dimensions, Modal, TouchableHighlight, TouchableWithoutFeedback, ToastAndroid, ActivityIndicator, Platform } from 'react-native';
...
export default class CoBuyers extends React.Component {
  ...
  deleteCoBuyerFromList() {
    ...
    if (this.state.items.some(e => e.buyers_id === this.state.buyers_id_for_delete)) {
      ...
      this.deleteCoBuyer(this.state.buyers_id_for_delete)
      //remove data from local
      if (index > -1) array.splice(index, 1);
    }
  }

  async deleteCoBuyer(buyers_id) {
            ...
            data.data.map((item, key) => {
              (item.co_buyers_first_name != "") ?
                ...
                this.state.items.push({
                  ...
                  co_buyers_email: item.co_buyers_pri_email,
                  co_buyers_phone: item.co_buyers_work_phone ? item.co_buyers_work_phone : item.co_buyers_home_phone ? item.co_buyers_home_phone : item.co_buyers_pri_phone_cell,
                })
  ...
  SearchFilterFunction(text) {
    ...
    const newData = this.state.itemsMirror.filter(function (item) {
      ...
      const itemPhone = item.co_buyers_phone ? item.co_buyers_phone.toUpperCase() : ''.toUpperCase();
      const itemEmail = item.co_buyers_email ? item.co_buyers_email.toUpperCase() : ''.toUpperCase();
      const itemFullname = itemFirst_name + ' ' + itemMid_name + ' ' + itemLast_name;

      const itemCoFirst_name = item.co_buyers_first_name ? item.co_buyers_first_name.toUpperCase() : ''.toUpperCase();
      const itemCoMid_name = item.co_buyers_mid_name ? item.co_buyers_mid_name.toUpperCase() : ''.toUpperCase();
      const itemCoLast_name = item.co_buyers_last_name ? item.co_buyers_last_name.toUpperCase() : ''.toUpperCase();
      const itemCoFullname = itemCoFirst_name + ' ' + itemCoMid_name + ' ' + itemCoLast_name;


      return itemCoFullname.indexOf(textName) > -1 || itemFullname.indexOf(textName) > -1 || itemFirst_name.indexOf(textName) > -1 || itemMid_name.indexOf(textName) > -1 || itemLast_name.indexOf(textName) > -1 || itemPhone.indexOf(textName) > -1 || itemEmail.indexOf(textName) > -1;
    });
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        ...
        <Text style={AppStyles.header_title_screen}>Co-Buyers</Text>
                ...
                <View style={{ flexDirection: "row", flex: 1 }}>
                  ...
                  <View style={{ flexDirection: "row", flex: 1 }}>
                    ...
                    <View style={{
                      ...
                      <Text style={styles.text}>Co-Buyer Name</Text>
                      <Text style={styles.text1} numberOfLines={1}>{item.co_buyers_first_name} {item.co_buyers_mid_name} {item.co_buyers_last_name}</Text>
                      <View style={{ flexDirection: "row" }}>
                        <Text style={styles.text} numberOfLines={1}>Phone Number : </Text>
                        <Text style={styles.text2} numberOfLines={1}>{item.co_buyers_phone}</Text>
                      </View>
                      <View style={{ flexDirection: "row" }}>
                        <Text style={styles.text}>Email : </Text>
                        <Text style={styles.text2}>{item.co_buyers_email}</Text>
                      </View>
                    </View>
                  ...
                  <View style={{
                    ...
                    <TouchableOpacity onPress={() => this.props.navigation.navigate("EditCoBuyerInfoScreen", { buyers_id: item.buyers_id })}>
                      <Image source={require('../../assets/images/edit_blue.png')} style={styles.cardicon} />
                    </TouchableOpacity>

                    <TouchableOpacity onPress={() => this.props.navigation.navigate("ViewCoBuyerInfoScreen", { buyers_id: item.buyers_id })}>
        ...
        {/* big orange button */}
        <TouchableOpacity style={AppStyles.buttonCard} activeOpacity={0.8} onPress={() => this.props.navigation.navigate("AddCoBuyerInfoScreen", { imageurl: '' })}>
          <Text style={AppStyles.buttonCardText}>Add Co-Buyers</Text>
          <Image
                ...
                <View>
                  ...
                  <View style={{ flexDirection: 'row' }}>
                    <TouchableOpacity
                      ...
                      activeOpacity={.5}
                      onPress={() => this.deleteCoBuyerFromList()}
                    >
                      <Text style={{
