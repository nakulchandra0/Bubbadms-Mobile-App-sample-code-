...
export default class Trade extends React.Component {
  ...
  toggleModalDelete(trade_inv_id) {
    this.setState({ isModalDeleteVisible: !this.state.isModalDeleteVisible, trade_inv_id_for_delete: trade_inv_id });
  };

  deleteTradeFromList() {
    this.setState({ isModalDeleteVisible: !this.state.isModalDeleteVisible });
    if (this.state.items.some(e => e.trade_inv_id === this.state.trade_inv_id_for_delete)) {
      var array = this.state.items;
      var index = array.findIndex(x => x.trade_inv_id === this.state.trade_inv_id_for_delete)
      //remove data from live database
      // console.log(array[index].id)
      this.deleteTrade(this.state.trade_inv_id_for_delete)
      //remove data from local
      if (index > -1) array.splice(index, 1);
    }
  }

  async deleteTrade(trade_inv_id) {
  ...
  addInventoryInfo() {
    ...
    if (this.state.vinText == "") {
      ...
      this.props.navigation.navigate('AddTradeInfoScreen', { vin: this.state.vinText })
  ...
  SearchFilterFunction(text) {
    ...
    const newData = this.state.itemsMirror.filter(function (item) {
      ...
      const itemData = item.trade_inv_vin ? item.trade_inv_vin.toUpperCase() : ''.toUpperCase();
      const textData = text.toUpperCase();

      const itemMake = item.trade_inv_make ? item.trade_inv_make.toUpperCase() : ''.toUpperCase();
      const textMake = text.toUpperCase();

      const itemModel = item.trade_inv_model ? item.trade_inv_model.toUpperCase() : ''.toUpperCase();
      const textModel = text.toUpperCase();

      const itemYear = item.trade_inv_year ? item.trade_inv_year.toUpperCase() : ''.toUpperCase();
      const textYear = text.toUpperCase();

      return itemData.indexOf(textData) > -1 || itemMake.indexOf(textMake) > -1 || itemModel.indexOf(textModel) > -1 || itemYear.indexOf(textYear) > -1;
    });
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        ...
        <Text style={AppStyles.header_title_screen}>Trade In</Text>
                ...
                <View style={{ flexDirection: "row", flex: 1 }}>
                  ...
                  <View style={{ flexDirection: "row", flex: 1 }}>
                    ...
                    <View style={{
                      ...
                      <Text style={styles.text1} numberOfLines={1}>{item.trade_inv_vin}</Text>
                      <View style={{ flexDirection: "row" }}>
                        <Text style={styles.text} numberOfLines={1}>Make & Model : </Text>
                        <Text style={styles.text2} numberOfLines={1}>{item.trade_inv_make} {item.trade_inv_model}</Text>
                      </View>
                      <View style={{ flexDirection: "row" }}>
                        <Text style={styles.text}>Year : </Text>
                        <Text style={styles.text2}>{item.trade_inv_year}</Text>
                      </View>
                    </View>
                    ...
                    <View style={{
                      ...
                      <Text style={styles.text1} numberOfLines={1}>${commafy(item.trade_inv_price)}</Text>
                    </View>
                  </View>
                  ...
                  <View style={{
                    ...
                    <TouchableOpacity onPress={() => this.props.navigation.navigate("EditTradeInfoScreen", { trade_inv_id: item.trade_inv_id })}>
                      <Image source={require('../../assets/images/edit_blue.png')} style={styles.cardicon} />
                    </TouchableOpacity>

                    <TouchableOpacity onPress={() => this.props.navigation.navigate("ViewTradeInfoScreen", { trade_inv_id: item.trade_inv_id })}>
                      <Image source={require('../../assets/images/view_blue.png')} style={styles.cardicon} />
                    </TouchableOpacity>

                    <TouchableOpacity onPress={this.toggleModalDelete.bind(this, item.trade_inv_id)}>
        ...
        <TouchableOpacity style={AppStyles.buttonCard} activeOpacity={0.8} onPress={this.toggleModal}>
          <Text style={AppStyles.buttonCardText}>Add Trade In</Text>
          <Image
          ...
          <TouchableWithoutFeedback onPress={this.toggleModal}>
            <View style={styles.centeredView}>
              <View style={styles.modalView}>
                <Text style={styles.modalText}>Add Trade In Info</Text>
                ...
                <View style={{ flexDirection: "row" }}>
                  ...
                  <TouchableOpacity style={{ position: "absolute", right: 0, bottom: 0 }} onPress={() => {
                    this.props.navigation.navigate("BarcodeScannerScreen", { screen: "AddTradeInfoScreen" })
                    this.setState({ isModalVisible: !this.state.isModalVisible, vinText: "", errorVINModel: false })
                  }}>
                    {/*Donute Button Image */}
                    <Image
                ...
                >
                  <Text style={styles.textStyle}>Add Trade In Info</Text>
                </TouchableOpacity>
              </View>
            </View>
              ...
              <TouchableHighlight style={styles.modalView}>
                <View>
                  ...
                  <Text style={{
                    ...
                    fontFamily: 'poppinsRegular',
                    marginBottom: 10
                  }}>Remove vehicle from trade!</Text>
                  ...
                  <View style={{ flexDirection: 'row' }}>
                    <TouchableOpacity
                      ...
                      activeOpacity={.5}
                      onPress={() => this.deleteTradeFromList()}
                    >
                      <Text style={{
